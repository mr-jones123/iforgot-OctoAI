...
function ScheduledPill({ scheduledAt }: { scheduledAt: string }) {
	const isOverdue = new Date(scheduledAt) < new Date();
	return (
		<div
			className={[
				"flex items-center gap-1.5 mt-1.5 px-2 py-1 rounded border",
				isOverdue
...
export function KanbanCard({
	...
	return (
		...
		>
			...
			{/* Scheduled time pill */}
			{card.scheduledAt && card.status === "idle" && (
				<ScheduledPill scheduledAt={card.scheduledAt} />
			)}
